...
void setup() {
  ...
  SerialUSB.println("Raw SerialFlash Hardware Test");
  SerialFlash.begin(FlashChipSelect);

  if (test()) {
    SerialUSB.println();
    SerialUSB.println("All Tests Passed  :-)");
    SerialUSB.println();
    SerialUSB.println("Test data was written to your chip.  You must run");
    SerialUSB.println("EraseEverything before using this chip for files.");
  } else {
    SerialUSB.println();
    SerialUSB.println("Tests Failed  :{");
    SerialUSB.println();
    SerialUSB.println("The flash chip may be left in an improper state.");
    SerialUSB.println("You might need to power cycle to return to normal.");
  }
}
...
bool test() {
  ...
  SerialUSB.println();
  SerialUSB.println("Read Chip Identification:");
  ...
  SerialUSB.print(" ");
  SerialUSB.println(buf[2], HEX);
  SerialUSB.print("  Part Nummber: ");
  SerialUSB.println(id2chip(buf));
  SerialUSB.print("  Memory Size:  ");
  chipsize = SerialFlash.capacity(buf);
  SerialUSB.print(chipsize);
  SerialUSB.println(" bytes");
  if (chipsize == 0) return false;
  SerialUSB.print("  Block Size:   ");
  blocksize = SerialFlash.blockSize();
  SerialUSB.print(blocksize);
  SerialUSB.println(" bytes");
  ...
  SerialUSB.println();
  SerialUSB.println("Reading Chip...");
  ...
  while (address < chipsize) {
    ...
    if (is_erased(buf, 8) == false) {
      if (equal_signatures(buf, sig) == false) {
        SerialUSB.print("  Previous data found at address ");
        SerialUSB.println(address);
        SerialUSB.println("  You must fully erase the chip before this test");
        SerialUSB.print("  found this: ");
        printbuf(buf, 8);
  ...
  if (count > 0) {
    ...
    while (address < chipsize) {
      ...
      if (is_erased(buf, 8)) {
        create_signature(address, sig);
        //SerialUSB.printf("write %08X: data: ", address);
        //printbuf(sig, 8);
        SerialFlash.write(address, sig, 8);
        while (!SerialFlash.ready()) ; // wait
        SerialFlash.read(address, buf, 8);
        if (equal_signatures(buf, sig) == false) {
          SerialUSB.print("  error writing signature at ");
          SerialUSB.println(address);
    ...
    SerialUSB.println("  all signatures present from prior tests");
  }
  ...
  SerialUSB.println();
  SerialUSB.println("Double Checking All Signatures:");
  memset(buf, 0, sizeof(buf));
  memset(sig, 0, sizeof(sig));
  memset(buf2, 0, sizeof(buf2));
  count = 0;
  ...
  while (address < chipsize) {
    ...
    create_signature(address, sig);
    if (equal_signatures(buf, sig) == false) {
      SerialUSB.print("  error in signature at ");
      SerialUSB.println(address);
      SerialUSB.print("  Read this: ");
      printbuf(buf, 8);
      SerialUSB.print("  Expected:  ");
  ...
  SerialUSB.print("  all ");
  SerialUSB.print(count);
  SerialUSB.println(" signatures read ok");


  // Read pairs of adjacent signatures
  // check read works across boundaries
  SerialUSB.println();
  SerialUSB.println("Checking Signature Pairs");
  memset(buf, 0, sizeof(buf));
  memset(sig, 0, sizeof(sig));
  memset(buf2, 0, sizeof(buf2));
  count = 0;
  ...
  while (address < chipsize - 8) {
    ...
    if (memcmp(buf, sig, 16) != 0) {
      SerialUSB.print("  error in signature pair at ");
      SerialUSB.println(address);
      SerialUSB.print("  Read this: ");
      printbuf(buf, 16);
      SerialUSB.print("  Expected:  ");
  ...
  SerialUSB.print("  all ");
  SerialUSB.print(count);
  SerialUSB.println(" signature pairs read ok");


  // Write data and read while write in progress
  SerialUSB.println();
  SerialUSB.println("Checking Read-While-Write (Program Suspend)");
  address = 256;
  ...
  if (address >= chipsize) {
    SerialUSB.println("  error, unable to find any blank space!");
    return false;
  }
  for (int i=0; i < 256; i += 8) {
    create_signature(address + i, sig + i);
  }
  SerialUSB.print("  write 256 bytes at ");
  SerialUSB.println(address);
  SerialUSB.flush();
  SerialFlash.write(address, sig, 256);
  usec = micros();
  if (SerialFlash.ready()) {
    SerialUSB.println("  error, chip did not become busy after write");
    return false;
  }
  SerialFlash.read(0, buf2, 8); // read while busy writing
  while (!SerialFlash.ready()) ; // wait
  usec = micros() - usec;
  SerialUSB.print("  write time was ");
  SerialUSB.print(usec);
  SerialUSB.println(" microseconds.");
  SerialFlash.read(address, buf, 256);
  if (memcmp(buf, sig, 256) != 0) {
    SerialUSB.println("  error writing to flash");
    SerialUSB.print("  Read this: ");
    printbuf(buf, 256);
    SerialUSB.print("  Expected:  ");
    printbuf(sig, 256);
  ...
  if (memcmp(buf2, sig, 8) != 0) {
    SerialUSB.println("  error, incorrect read while writing");
    SerialUSB.print("  Read this: ");
    printbuf(buf2, 256);
    SerialUSB.print("  Expected:  ");
    printbuf(sig, 256);
  ...
  SerialUSB.print("  read-while-writing: ");
  printbuf(buf2, 8);
  SerialUSB.println("  test passed, good read while writing");
  ...
  if (chipsize >= 262144 + blocksize + testIncrement) {
    SerialUSB.println();
    SerialUSB.println("Checking Read-While-Erase (Erase Suspend)");
    memset(buf, 0, sizeof(buf));
    memset(sig, 0, sizeof(sig));
    memset(buf2, 0, sizeof(buf2));
    ...
    if (SerialFlash.ready()) {
      SerialUSB.println("  error, chip did not become busy after erase");
      return false;
    }
    SerialFlash.read(0, buf2, 8); // read while busy writing
    while (!SerialFlash.ready()) ; // wait
    usec = micros() - usec;
    SerialUSB.print("  erase time was ");
    SerialUSB.print(usec);
    SerialUSB.println(" microseconds.");
    ...
    while (address < chipsize) {
      ...
      if (address >= 262144 && address < 262144 + blocksize) {
        if (is_erased(buf, 8) == false) {
          SerialUSB.print("  error in erasing at ");
          SerialUSB.println(address);
          SerialUSB.print("  Read this: ");
          printbuf(buf, 8);
          return false;
    ...
    SerialUSB.print("  erase correctly erased ");
    SerialUSB.print(blocksize);
    SerialUSB.println(" bytes");
    // now check if the data we read during erase is good
    create_signature(0, sig);
    if (memcmp(buf2, sig, 8) != 0) {
      SerialUSB.println("  error, incorrect read while erasing");
      SerialUSB.print("  Read this: ");
      printbuf(buf2, 256);
      SerialUSB.print("  Expected:  ");
      printbuf(sig, 256);
    ...
    SerialUSB.print("  read-while-erasing: ");
    printbuf(buf2, 8);
    SerialUSB.println("  test passed, good read while erasing");

  } else {
    SerialUSB.println("Skip Read-While-Erase, this chip is too small");
  }
...
void printbuf(const void *buf, uint32_t len)
{
  ...
  do {
    unsigned char b = *p++;
    SerialUSB.print(b >> 4, HEX);
    SerialUSB.print(b & 15, HEX);
    //SerialUSB.printf("%02X", *p++);
    SerialUSB.print(" ");
  } while (--len > 0);
  SerialUSB.println();
}
